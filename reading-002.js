...
for (let key of ['bndl1', 'bndl2', 'bndl3', 'bndl4', 'bndl5', 'bndl6']){
    ...
    const stream = fs.createWriteStream(`inter-${key}.geojsons`, fsOptions)
    ...
    stream.on('finish', () => {
        console.log(`Stream ends for ${key}.`)
        //Tippecanoe after FS write

        tippecanoe = spawn(tippecanoePath, [
            `--output=${key}-test.mbtles`,
            '--minimum-zoom=1',
            '--maximum-zoom=4',
            `inter-${key}.geojsons`
        ...
        tippecanoe.on('exit',() => {
            console.log(`tippecanoe ends for ${key}`)
        }
        )
        //
    })
